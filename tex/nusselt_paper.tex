...
At low to moderate Ra, Nu $\propto$ Ra$_{\text{top}}^{1/3}$ regardless of $\epsilon$ (NEED TO CITE A
RB PAPER WITH A 1/3 POWER LAW \cite{ahlers&all2009} maybe?).  At large Ra$_{\text{top}}$, Nu $\propto$ Ra$_{\text{top}}^{1/5}$ at $\epsilon = 0.5$.
The scaling of Nu with Ra is unclear at low $\epsilon$ (so I'll try to fill in that space with
...
where Ma $\geq 1$ for $\epsilon = 0.5$, this scaling gives way to a Re $\propto$ Ra$_{\text{top}}^{1/2}$.
...
As these simulations evolve and the thermodynamic variables evolve, the density
...
convection,  both in determining the utility of simplified equation sets (such as the
anelastic equations) and in coming to understand more realistic systems, 
