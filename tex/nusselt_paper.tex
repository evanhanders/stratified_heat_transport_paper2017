...
Throughout this letter we set $n_{\rho} = 3$.  The polytropic
index is $m = m_{ad} - \epsilon$, where $m_{ad} = (\gamma - 1)^{-1}$
and $\epsilon$ is the superadiabatic excess.  Satisfying hydrostatic
...
The superadiabaticity is set by $\epsilon$ and 
the initial entropy
...
constant with depth.  Throughout this work we specify
that Pr $= 1$ and is depth invariant.  By these
choices, $\chi(z) = \chi_t / \rho_0$ and $\nu(z) = \nu_t / \rho_0$.
This formulation 
...
thus increasing the ratio of the diffusive timescales to the
buoyant timescale.
