...
superadiabaticity ($\epsilon$) and Ra at a set
initial density stratification ($n_{\rho}$), aspect
ratio, and Prandtl number (Pr).
We find that Ma is a strong function of $\epsilon$
in both 2D and 3D simulations, and that it is a 
weak function of Ra in 2D.
The scaling of evolved fluid properties (Nusselt number, Reynolds number,
stratification) differs in the subsonic and supersonic regimes; however,
aside from the sonic transition, the evolved flow properties
of low and high Ma flows are nearly identical.  Our limited 3D runs suggest that
evolved fluid properties behave similarly in 2D and 3D, but with modified
scaling laws.
\end{abstract}
\maketitle
...
a significant temperature gradient and conductive flux persist despite
...
We have observed interesting phenomena, such as time-dependent oscillating shearing windy
states in our 2D simulations.  Unlike in RB \cite{goluskin&all2014}, our solutions with
